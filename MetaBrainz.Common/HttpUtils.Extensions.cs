...
public static partial class HttpUtils {
  ...
  /// <summary>Convenience extension methods for HTTP content headers.</summary>
  /// <param name="contentHeaders">The headers to get the information from.</param>
  extension(HttpContentHeaders contentHeaders) {
    ...
    /// </returns>
    public string GetContentEncoding() => contentHeaders.GetContentEncoding("utf-8");

    /// <summary>Gets the content encoding based on content headers.</summary>
    /// <param name="defaultEncoding">The value to return when no encoding could be determined based on the headers.</param>
    /// <returns>
    /// The content encoding extracted from the headers (first from the <c>Content-Encoding</c> header, then from a <c>charset</c>
  ...
  /// <summary>Convenience extension methods for HTTP response messages.</summary>
  /// <param name="response">The response to process.</param>
  extension(HttpResponseMessage response) {

    /// <summary>Checks a response to ensure it was successful.</summary>
    /// <param name="cancellationToken">The cancellation token to cancel the operation.</param>
    /// <returns>The response, unchanged.</returns>
    /// <exception cref="HttpError">When the response did not have a successful status.</exception>
    public async ValueTask<HttpResponseMessage> EnsureSuccessfulAsync(CancellationToken cancellationToken = new()) {
    ...
    public async Task<string> GetStringContentAsync(CancellationToken cancellationToken = new()) {
